...
//Creating the folder
	...
	//std::string path_name = "${ROS_WORKSPACE}/save_Demonstrations/" + folder_name + "/";
...
//		printf("sonar Y  %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f \n",sensor.sonar_y[0],sensor.sonar_y[1],sensor.sonar_y[2],sensor.sonar_y[3],sensor.sonar_y[4],sensor.sonar_y[5],sensor.sonar_y[6],sensor.sonar_y[7],sensor.sonar_y[8],sensor.sonar_y[9],sensor.sonar_y[10],sensor.sonar_y[11],sensor.sonar_y[12],sensor.sonar_y[13],sensor.sonar_y[14],sensor.sonar_y[15]);
		...
		{		
			...
			if(distance_Max!=0)
			{
				...
				{
				out_file << std::setw(20) << "Initial tilt angle" << tilt_angle << "\n";
				out_file << std::setw(20) << "Max Distance (Initial distance)" << distance_Max << "\n";
				out_file << std::setw(20) << "Min Distance" << distance_Min << "\n\n";
				out_file << std::setw(20) << "translational vel" 
					 << std::setw(20) << "rotational vel"
					 << std::setw(20) << "delta Pan angle"
					 << std::setw(20) << "time" << "\n";
				}
			}			
		}
...
//				vel.linear=translational_velocity;
		...
		if((listener.canTransform("/head_1", "/openni_depth_frame", ros::Time(0), NULL)==false) && (flag_calibration==true)) 
		{				
			rotational_velocity=0;
			translational_velocity=0;
			vel.angular.z = 0;
...
//		printf("  set to the robot  is  %f vs  %f\n", vel.angular.z , vel.linear.x);
		...
		msgs[7] = rotational_velocity;
		msgs[8] =  -1 * rotational_velocity*PI/180;
...
//stop Gesture
		if ((fabs(left_hand_IM[0]-right_hand_IM[0])<30) && (fabs(left_hand_IM[1]-right_hand_IM[1])<30) && left_hand_CF[0]!=0 && left_hand_CF[1]!=0 && right_hand_CF[0]!=0 && right_hand_CF[1]!=0)
		{
			vel.angular.z = 0;
			vel.linear.x = 0;
			robotrotationvel.publish(vel);	
		...
		if(listener.canTransform("/head_1", "/openni_depth_frame", ros::Time(0), NULL)==true)
		{
			...
			else 
			{
				out_file << std::setw(20) <<  translational_velocity
					 << std::setw(20) << rotational_velocity
...
const double init_tilt(double y, double d)
{
	...
	d = sqrt((d*d)-(y*y));//real
	//printf("init %f 	 %f\n",init,d);
	double tilt_angle = (atan(z/d)-atan(z_/d))*180/PI+5;
	return tilt_angle;
